...
from wm_dashboard.forecast import (
    DEFAULT_MODEL as FORECAST_DEFAULT_MODEL,
)
from wm_dashboard.forecast import (
    ForecastError,
    ForecastResult,
...
def page_forecast() -> None:
    ...
    st.markdown(
        "<div class='tag-sourced'>"
        "One-week forward view generated by Claude from public market data. "
        "Treat as one input among many; the IPS still requires a human pre-mortem before any trade."
        "</div>",
        unsafe_allow_html=True,
    )

    portfolio = _load_portfolio()
    held_tickers = sorted({p.ticker for p in portfolio.positions if not p.is_cash})
    ...
    with st.form("forecast_form", clear_on_submit=False):
        ...
        with c1:
            choice = st.selectbox(
                "Ticker",
                options=held_tickers + ["(other ticker)"],
                index=0,
            )
            other_ticker = st.text_input(
                "Other ticker (used when '(other ticker)' is selected)",
        ...
        with c2:
            model_choice = st.selectbox(
                ...
                if FORECAST_DEFAULT_MODEL
                in {"claude-haiku-4-5", "claude-sonnet-4-6", "claude-opus-4-7"}
                else 0,
            )
        submitted = st.form_submit_button("Generate forecast", width="stretch")

    target = (other_ticker if choice == "(other ticker)" else choice).upper().strip()

    # Display any previously-generated forecasts in this session.
    cached: dict[str, ForecastResult] = st.session_state.setdefault("forecast_cache", {})
    if submitted and target:
        with st.spinner(f"Generating {target} forecast via {model_choice}..."):
            try:
                result = run_forecast(target, model=model_choice)
                cached[target] = result
            except ForecastUnavailable as exc:
                st.markdown(
                    f"<div class='card card-breach'>{style.status_pill('BREACH')} "
                    f"Forecast unavailable: {exc}</div>",
                    unsafe_allow_html=True,
                )
                return
                ...
                st.markdown(
                    f"<div class='card card-breach'>{style.status_pill('BREACH')} "
                    f"Forecast generation failed: {exc}</div>",
                    unsafe_allow_html=True,
                )
                return

    if not cached:
        st.info("Pick a ticker and click Generate forecast.")
        return

    # Newest first, with a clear button to wipe the session cache.
    if st.button("Clear all forecasts in this session", key="forecast_clear"):
        st.session_state["forecast_cache"] = {}
        st.rerun()

    for ticker_key in reversed(list(cached.keys())):
        st.markdown(_render_forecast_card(cached[ticker_key]), unsafe_allow_html=True)
...
def main() -> None:
    ...
    pages = [
        ...
        st.Page(page_forecast, title="Forecast"),
